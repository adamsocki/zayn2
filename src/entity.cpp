...
void AllocateEntityManager(EntityManager *manager, MemoryArena *arena, int32 *entityTypeCounts)
{
    ...
    for (int i = 0; i < EntityType_Count; i++)
    {
        ...
        *storage = {};
        ...
        {
            totalEntityCount += entityTypeCounts[i];
...
{
    if (handle.id >= manager->entityInfo.count) 
    { 
        return NULL; 
    }
